        ...
        switch (state) {
           ...
           // Relational & Assignment Operators
            case 24:
                ...
                else if (c == '=') { T.lexeme[i++] = c; T.type = TK_LE; return T; }
            ...
            case 26:
                if (c == '-') { T.lexeme[i++] = c; T.type = TK_ASSIGNOP; return T; }
                else { retract(); T.type = TK_ERROR; T.err = ERR_UNKNOWN_PATTERN; return T; }
                break;
            ...
            case 34:
                if (c == '&') { T.lexeme[i++] = c; T.type = TK_AND; return T; }
                else { retract(); T.type = TK_ERROR; T.err = ERR_UNKNOWN_PATTERN; return T; }
                break;
            ...
            case 37:
                if (c == '@') { T.lexeme[i++] = c; T.type = TK_OR; return T; }
                else { retract(); T.type = TK_ERROR; T.err = ERR_UNKNOWN_PATTERN; return T; }
                break;

            // Equality and Relational (==, !=, >=)
            case 30:
                if (c == '=') { T.lexeme[i++] = c; T.type = TK_EQ; return T; }
                else { retract(); T.type = TK_ERROR; T.err = ERR_UNKNOWN_PATTERN; return T; }
                break;

            case 42:
                if (c == '=') { T.lexeme[i++] = c; T.type = TK_NE; return T; }
                else { retract(); T.type = TK_ERROR; T.err = ERR_UNKNOWN_PATTERN; return T; }
                break;

            case 45:
                if (c == '=') { T.lexeme[i++] = c; T.type = TK_GE; return T; }
